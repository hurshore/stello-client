...
const checkoutForm = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${baseApiUrl}/payment/create-payment-intent`, {
      headers: {
  ...
  const handleSubmit = async (ev) => {
    ...
    if (payload.error) {
      ...
      setSucceeded(true);
      try {
        const res = await fetch(`${baseApiUrl}/cart/all`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
  ...
  return (
    <div className={classes.checkoutForm}>
      ...
      <form id="payment-form" onSubmit={handleSubmit}>
        ...
        <button
          disabled={
            processing || disabled || succeeded || cartState.total === 0
          }
          id="submit"
        >
          <span id="button-text">
        ...
        <p
          className={
            succeeded
              ? classes.resultMessage
              : `${classes.resultMessage} ${classes.hidden}`
          }
        >
          Payment succeeded, thanks for doing business with us. Back to{' '}
          <Link href="/shop">
            <a>shop</a>
          </Link>
        </p>
